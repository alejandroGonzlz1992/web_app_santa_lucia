...
class Email_Manager:
    ...
    async def send_extra_hours_request_notification(
            self, record: Union[object, dict], rec: Union[list[str], str]) -> str:
        # header
        msg = self.builder["multipart"]("alternative")
        msg["From"] = self.cns.EMAIL_SANTALUCIA_SENDER.value
    ...
    # extra hours request update
    async def send_extra_hours_update_request_notification(
            self, record: Union[object, dict], rec: Union[list[str], str]) -> str:
        # header
        msg = self.builder["multipart"]("alternative")
        msg["From"] = self.cns.EMAIL_SANTALUCIA_SENDER.value
        msg["Subject"] = self.cns.SUBJECT_PERMISSION_EXTRA_HOURS_UPDATE_REQUEST.value
    ...
    # vacations request send
    async def send_vacations_request_notification(
            self, record: Union[object, dict], rec: Union[list[str], str]) -> str:
        # header
        msg = self.builder["multipart"]("alternative")
        msg["From"] = self.cns.EMAIL_SANTALUCIA_SENDER.value
        msg["Subject"] = self.cns.SUBJECT_PERMISSION_VACATIONS_REQUEST.value

        # html content
        content = self.builder["text"](
            self.template.html_permission_vacations_request(
                record=record, login_url=self.cns.EMAIL_TO_LOGIN_SESSION.value), "html")
    ...
    # vacations request update
    async def send_vacations_update_request_notification(
            self, record: Union[object, dict], rec: Union[list[str], str]) -> str:
        # header
        msg = self.builder["multipart"]("alternative")
        msg["From"] = self.cns.EMAIL_SANTALUCIA_SENDER.value
        msg["Subject"] = self.cns.SUBJECT_PERMISSION_VACATIONS_UPDATE_REQUEST.value
    ...
    # inability request send
    async def send_inability_request_notification(
            self, record: Union[object, dict], rec: Union[list[str], str]) -> str:
        # header
        msg = self.builder["multipart"]("alternative")
        msg["From"] = self.cns.EMAIL_SANTALUCIA_SENDER.value
        msg["Subject"] = self.cns.SUBJECT_INABILITY_REQUEST.value

        # html content
        content = self.builder["text"](
            self.template.html_inability_submission_request(
                record=record, login_url=self.cns.EMAIL_TO_LOGIN_SESSION.value), "html")
